# NovaTech · Assistente de IA com RAG — Recorte de Domínio (final)

| Item | Conteúdo |
| --- | --- |
| Documento | 05-bounded-contexts-final (substitui 01-bounded-contexts-v1, rev. 1.1) |
...
| Data | 24/09/2026 |
| Status | Final, com decisões pendentes explícitas (§4.1) e validação com Produto, Comercial, Operações, Compliance e Atendimento |
| Revisão aplicada | `04-tech-lead-review.md` · histórico em `07-historico-iteracao.md` |
| Consumido por | `06-requirements-final.md` |
...
| Final | 24/09/2026 | rev. 1.1 + `04-tech-lead-review.md` | Correções da revisão do Tech Lead, com os IDs `T-01` a `T-40` preservados. Mudanças principais abaixo |

**Principais mudanças da versão final em relação à rev. 1.1**

- **IDs preservados.** `T-01` a `T-40` mantêm número e sentido. Um único termo de negócio novo: `T-41` (chamado). Os termos do produto (§3.3) ganham IDs `TP-01` a `TP-10`, na ordem original, e os termos novos do produto seguem de `TP-11` a `TP-40`.
- **Novas seções de propriedade** (§2.3 a §2.6): dono único de cada conceito que o requirements tratava em mais de um lugar, tabela tema → área de destino, catálogo de temas sensíveis e registro de conflitos por seção.
- **BC-01** passa a produzir o registro de interação e o identificador da orientação, que o BC-03 e o BC-04 referenciam; sinaliza ao BC-02 divergências não registradas.
- **BC-03** passa a ser dono do catálogo de temas sensíveis e da tabela de áreas.
- **BC-07** passa a ser dono do alerta de carga perigosa em todos os contexts; a área se divide entre Gestão de Riscos (exceção de devolução) e Compliance (frete, tarifa e expresso).
- **T-26** ganha regra para classes fora da lista da POL-001; **T-41** registra a polissemia de "chamado".
- **Regra de uso do glossário** (§3): os valores citados aqui servem para desambiguar termos; a orientação só pode citar o Anexo A.
- **Situação da resposta** passa a ter IDs S0 a S9 (TP-17), mantendo os nomes da rev. 1.1 (completa, parcial, sem evidência, baixa evidência, conflito).
- **Verificação contra o Anexo A e os entregáveis da fase 1:** tabela de áreas revisada com a proposta da jornada da Tarefa 1; data e status de vigência exibidos conforme o cabeçalho de cada documento (TP-27, TP-41); rótulo da fonte informal alinhado ao cabeçalho do FAQ; nota sobre classificação de incidente crítico (T-22).
...
| `JORNADA` | Jornada integrada do atendente (PDF, `Output_Tarefa2_Jornada_integrada`) | — | Produto | Artefato de produto: princípio central, guardrails G1–G6, raias e caminhos. A jornada textual da Tarefa 1 (guardrails G1–G9) está superada por esta; em caso de divergência, vale o PDF |
...
- Situação da resposta: completa, parcial, sem evidência, baixa evidência, conflito, com IDs S0 a S9 (TP-17; tabela em `06-requirements-final.md` §3.9). Situação e nível de confiança por parte da dúvida (TP-14).
- Nível de confiança baseado na qualidade da evidência.
- Citação de documento, seção, versão e vigência (`JORNADA` G3).
- Reconhecimento de conflito entre fontes, mostrando as duas sem escolher sozinho (`JORNADA` G4): aplica o registro de conflitos do BC-02 (§2.6) e sinaliza ao BC-02 a divergência detectada que não consta do registro (TP-32).
- Reconhecimento de tema sensível (`JORNADA` G5), pelo catálogo do BC-03 (§2.5), e indicação da área de destino pela tabela do BC-03 (§2.4).
...
- Registro de concordância ou discordância do atendente, vinculado ao identificador da orientação, com os motivos da `JORNADA` R1. É uma operação própria do BC-01, fora do query endpoint especificado em `06-requirements-final.md`.
- Registro de interação (TP-36) e identificador da orientação (TP-37): registro único por orientação, referenciado pelo BC-03 e pelo BC-04.
...
**Termos principais:** situação da resposta (TP-17), evidência, confiança (TP-18), interpretação permitida (TP-08), inferência (TP-09), data de referência (TP-04), parte (TP-14), bloco (TP-15).
...
- **BC-01 → BC-03 e BC-04 (Customer-Supplier):** indicação de escalonamento e registro de interação, pelo identificador.
- **BC-03 → BC-01 (Conformist):** catálogo de temas sensíveis e tabela de áreas.
- **BC-01 → BC-02 (evento de pendência):** divergência detectada não registrada, para triagem.
...
- Registro de conflitos conhecidos (`Anexo A · Notas`, contradições 1 a 4, e POL-001 × FAQ #38), na granularidade de **seção** (§2.6).
- Triagem das divergências detectadas pelo BC-01 que não constam do registro (TP-32).
...
- Dono do **catálogo de temas sensíveis** (§2.5) e da **tabela tema → área de destino** (§2.4), incluindo a área padrão de fallback (TP-35).
- Caso de escalonamento vinculado ao registro de interação produzido pelo BC-01 (TP-36), pelo identificador; o BC-03 acrescenta motivo recebido, desfecho e resposta validada, sem manter registro paralelo da orientação.

**Fora:**
- A decisão de negócio, que é da área dona.
- A correção da documentação (BC-04 com o dono do documento).

**Termos principais:** tema sensível (TP-06), escalonamento (TP-10), indicação de escalonamento (TP-40), área de destino (TP-34), validação humana (TP-33); Gestão de Riscos (T-35).
...
- Feedback registrado com pergunta, resposta, fonte e versão (`JORNADA` R2), obtidos do registro de interação pelo identificador da orientação (TP-37).
...
- **BC-09 → BC-05 (Customer-Supplier):** regra de desconto por volume (T-13), que na v2 incide sobre o multiplicador regional; o contrato pode determinar a tabela (`FAQ #8`, informal, sem respaldo formal).
...
- **Dono do alerta de carga perigosa** em todos os contexts (validação humana sempre) e das linhas de carga perigosa da tabela de áreas (§2.4).
- Menção a classe de risco fora das classes 1 a 6 ou a produto perigoso sem classe: o assistente não classifica como incluída nem excluída e aplica o alerta (ver T-26).
...
### 2.3 Propriedade de conceitos

Resolve os pontos em que o requirements v1 atribuía o mesmo conceito a mais de um context. Onde a rev. 1.1 já definia o dono, a linha apenas o confirma.

| Conceito | Dono único | Consumidores | Situação na rev. 1.1 |
| --- | --- | --- | --- |
| Tier e critério de tier (T-02) | BC-09 | BC-08 (Conformist), BC-01 | Já definido; confirmado |
| Desconto por volume (T-13) | BC-09 | BC-05, BC-01 | Já definido; confirmado |
| Carga perigosa: definição, alerta e área (T-26) | BC-07 | BC-05, BC-06, BC-08, BC-10, BC-11, BC-01 | Dono já definido; alerta e área acrescentados |
| Avaria em trânsito (T-37) | Fronteira BC-10 / BC-11 em conflito (pendência 4) | BC-01 | Mantido. Para a orientação, a regra formal (POL-001 §3.5, BC-10) prevalece sobre o FAQ #38 (C-04 do requirements) e o conflito é exposto |
| Registro de conflitos | BC-02 | BC-01 | Já definido; granularidade por seção acrescentada (§2.6) |
| Divergência detectada não registrada (TP-32) | BC-01 sinaliza; BC-02 faz a triagem | — | Novo |
| Catálogo de temas sensíveis (TP-06) | BC-03 | BC-01 | Implícito; explicitado |
| Tabela tema → área de destino (TP-34) | BC-03 | BC-01 | Destinos documentados já listados no BC-03; tabela completa acrescentada (§2.4) |
| Registro de interação e identificador (TP-36, TP-37) | BC-01 (produz) | BC-03 e BC-04, pelo identificador | Na rev. 1.1, o "registro do caso" estava no BC-03; passa a ser único e produzido pelo BC-01 |
| Concordância ou discordância do atendente (TP-38) | BC-01 (registra) | BC-04 (trata depois de registrado) | Já definido; confirmado |
| Situação da resposta e confiança (TP-17, TP-18) | BC-01 | — | IDs S0 a S9 acrescentados |

### 2.4 Tabela tema → área de destino (dono: BC-03)

Uma única área por parte com escalonamento indicado. As linhas com base formal vêm dos destinos que o Anexo A já nomeia (BC-03, "Destinos que o próprio Anexo A nomeia"). As demais seguem a tabela "Escalonamento: regras comuns" da jornada da Tarefa 1 (proposta de produto, status "para validação"). A tabela é provisória até a DP-30 e a DT-02; as linhas de carga perigosa dependem também da DT-10. Quando mais de uma linha se aplica, vale a primeira da tabela.

| # | Tema da parte | Área de destino | Base |
| --- | --- | --- | --- |
| 1 | Carga perigosa em devolução (exceção à devolução padrão) | Gestão de Riscos, ramal 4500 | `POL-001 §3.2` (formal) |
| 2 | Carga perigosa em frete, tarifa (PROC-043), frete expresso ou qualquer outro tema | Compliance | `PROC-042 v2 §4` (PROC-043 em revisão pelo Compliance); `FAQ #32` (informal). A Tarefa 1 propõe "Operações e Compliance" (duas áreas): decisão em DT-10 |
| 3 | Carga acima de 5.000 kg | Operações (gerente de operações regional) | `PROC-042 v1 §4`, `v2 §4` (formal) |
| 4 | Pedido de desconto acima dos percentuais da v2 | Diretoria Comercial | `PROC-042 v2 §4` (formal) |
| 5 | Desconto, pedido de concessão, condições comerciais, tier, pedido de SLA fora dos tiers | Comercial | `PROC-042 v1 §4`; `SLA-2024 §1`, nota (formal) |
| 6 | Devolução com prazo expirado | Comercial | `POL-001 §3.5` (formal) |
| 7 | Frete especial (enquadramento, fórmula, multiplicador, fator de peso, valor) e frete padrão | Comercial | Dono do PROC-042: Diretoria Comercial (§1) |
| 8 | Prazo de entrega, prazo adicional, prazo padrão da rota, interceptação | Operações | Dono do BC-06 (§1); Tarefa 1 ("Prazos, rotas e aprovação acima de 5.000 kg") |
| 9 | SLA, incidente crítico, penalidades | Área padrão de fallback | Sem destino único documentado (dono conjunto do SLA-2024). Indício de incidente crítico segue o fluxo de incidente do SLA pelo atendente (Tarefa 1, P1) |
| 10 | Devolução: demais exceções e avaria em trânsito tratada como devolução (`POL-001 §3.5`) | Operações | Dona da POL-001: Diretoria de Operações (§1); Tarefa 1 ("Devolução → Operações") |
| 11 | Carga danificada e sinistro (processo do `FAQ #38`) | Jurídico (sinistros@novatech.com.br, a confirmar) | `FAQ #38` (informal); Tarefa 1 ("Carga danificada e sinistro → Jurídico"); BC-11 sem dono formal (pendência 10) |
| 11a | Seguro de carga | Comercial | `FAQ #22` ("confirme com o Comercial", informal); Tarefa 1, exemplo do F2 |
| 12 | Sem evidência sem tema identificável | Área padrão de fallback | DP-30 |
| — | **Área padrão de fallback (TP-35)** | Supervisão de Atendimento (SAC N2), **provisório** | DP-30. Base: raia 5 da `JORNADA` ("Supervisão, SAC N2 e donos da documentação") e guardrail (3) do exercício ("sugerir escalar para o supervisor") |

### 2.5 Catálogo de temas sensíveis (dono: BC-03)

Baseline: avaria, extravio, indenização, carga perigosa, reclamação formal e exceção contratual (`JORNADA` G5), mais descontos, seguro, exceções de devolução e incidentes críticos (baseline da ESPEC-V2). Todos exibem "Requer validação humana". Os níveis N1, N2 e N3 por tema dependem da DP-08.

### 2.6 Registro de conflitos (dono: BC-02)

A granularidade é a **seção** do documento. Um conflito afeta apenas as seções listadas; as demais seções do mesmo documento não são rebaixadas por ele.

| Conflito | Seções | Origem |
| --- | --- | --- |
| Multiplicadores regionais v1 × v2 | `PROC-042 v1 §2.1` × `v2 §2.1` (regra de transição em `v2 §5`) | Contradição 1; T-10, T-12 |
| Fator de peso v1 × v2 | `PROC-042 v1 §2` × `v2 §2` | Contradição 2; T-11 |
| Prazo adicional v1 × v2 | `PROC-042 v1 §3` × `v2 §3` | Contradição 3; T-16 |
| Desconto por volume v1 × v2 × FAQ | `PROC-042 v1 §4` × `v2 §4` × `FAQ #45` | T-13 |
| Versão aplicável pelo contrato | `PROC-042 v2 §5` × `FAQ #8` | T-12 |
| Avaria em trânsito | `POL-001 §3.5` × `FAQ #38` | Achado adicional; T-37 |
| Frete expresso de carga perigosa sem documento formal | `FAQ #32` (sem fonte formal) | Contradição 4; T-07, T-26 |

A seção 2 é citada tanto para a fórmula (idêntica nas duas versões, sem conflito) quanto para o fator de peso (em conflito). O conflito atinge apenas a tabela de fatores de peso; a fórmula pode ser citada sem alerta de divergência.

---

## 3. Linguagem ubíqua

**Regra de uso do glossário:** os valores reproduzidos aqui (prazos, percentuais, multiplicadores) existem para desambiguar os termos e sempre trazem a seção de origem. O glossário **não é fonte de evidência**: a orientação só cita o Anexo A, e, se houver divergência entre este glossário e o Anexo A, prevalece o Anexo A. Qualquer atualização de um documento do Anexo A exige revisar os termos que citam aquela seção.
...
| T-41 | Chamado (polissemia) | BC-08 / BC-10 / BC-05 | AMBIGUIDADE | SLA-2024 §5; POL-001 §3.3; PROC-042 v2 §5 |
...
- **Lacuna (AMBIGUIDADE):** o objetivo diz "acima de 500kg", mas a primeira faixa do fator de peso vai "de 500kg a 1.000kg" nas duas versões; não se sabe se exatamente 500 kg entra. O assistente não resolve a ambiguidade: exibe os dois trechos e indica o Comercial.
...
- **Bounded context:** BC-08.
- **Regra de produto:** o assistente informa os critérios e os prazos; pode dizer que os dados informados atendem a um critério explícito (interpretação permitida), mas não classifica o chamado como incidente crítico. A classificação segue o fluxo de incidente do SLA pelo atendente (jornada da Tarefa 1, P1 e G6).
...
- **Regra para classes não listadas:** quando a dúvida menciona outra classe de risco ou um produto perigoso sem classe informada, o assistente não o classifica como incluído nem excluído da definição, não usa a regulamentação ANTT para decidir e exibe "Requer validação humana".
- **Interpretação incorreta provável:** estender a outras classes da regulamentação ANTT além das 1 a 6 listadas; concluir, pelo silêncio da POL-001, que outra classe não é perigosa; dizer que devolução é "impossível" (o FAQ #3 orienta dizer que exige tratamento especial); tratar a autorização do Compliance para expresso como regra formal.
...
#### T-41 · Chamado (polissemia)
- **Definição oficial:** **AMBIGUIDADE.** O termo aparece em três documentos com sentidos diferentes:
  - **chamado de atendimento**: registro no sistema de chamados cujo tempo é medido pelo SLA a partir da abertura, como chamado geral (T-21) ou incidente crítico (T-22) (`SLA-2024 §2, §5`);
  - **chamado de devolução**: solicitação aberta no Portal do Cliente com CT-e, fotos e motivo, que inicia a triagem (`POL-001 §3.3`);
  - **chamado da regra de transição**: chamado cuja data de abertura escolhe os multiplicadores regionais (`PROC-042 v2 §5`); o documento não diz se é um chamado de atendimento, uma solicitação de cotação ou outro registro.
- **Bounded context:** BC-08, BC-10 e BC-05.
- **Interpretação incorreta provável:** usar a data de abertura do chamado de atendimento em curso como data de referência do PROC-042 sem confirmar que se refere ao frete em questão; tratar a triagem da devolução como primeira resposta de SLA.
- **Fonte:** `SLA-2024 §2, §5`; `POL-001 §3.3`; `PROC-042 v2 §5`. Decisão pendente: DP-12 e DT-11.
...
| ID | Termo | Definição (ESPEC-V2, seção 4) | Context | Interpretação incorreta provável |
| --- | --- | --- | --- | --- |
| TP-01 | Fonte oficial (formal) | Documento formalmente aprovado e reconhecido como referência | BC-02 | Considerar oficial tudo o que está na base |
| TP-02 | Fonte informal | Material de apoio sem autoridade equivalente, como o FAQ | BC-02 | Tratar o FAQ como oficial porque cita a PROC-042 |
| TP-03 | Documento sem vigência confirmada | Documento formal sem evidência de que se aplica à data de referência | BC-02 | Tratar a ausência de "obsoleto" como "vigente" (caso das duas versões do PROC-042) |
| TP-04 | Data de referência | Data à qual a pergunta se aplica | BC-02 / BC-01 | Usar a data de hoje, quando a v2 §5 usa a data de abertura do chamado |
| TP-05 | Conflito documental | Divergência de valor, prazo, critério, percentual, elegibilidade ou procedimento entre fontes aplicáveis | BC-02 / BC-01 | Escolher a fonte mais recente ou mais detalhada |
| TP-06 | Tema sensível | Assunto cujo erro gera impacto relevante e exige tratamento adicional | BC-03 | Tratar como questão de tom, e não como gatilho de validação |
| TP-07 | Baixa evidência | A única informação é informal, incompleta, ambígua ou insuficiente. Situação S4 (TP-17). Não confundir com baixa confiança, que é nível (TP-18). | BC-01 | Responder com ressalva genérica |
| TP-08 | Interpretação permitida | Aplicação direta de regra explícita a dado fornecido. Resulta em nível no máximo Médio. Exemplo positivo: aplicar o "OU" do critério Gold (T-02) aos valores informados. Exemplo negativo: classificar um produto por norma externa. | BC-01 | Considerar permitida qualquer dedução "razoável" |
| TP-09 | Inferência | Conclusão que acrescenta algo não escrito na fonte; proibida. Exemplo: concluir que a classe 8 está excluída porque a POL-001 §3.2 lista as classes 1 a 6. | BC-01 | Completar lacunas com conhecimento geral ou regulação externa |
| TP-10 | Escalonamento | Encaminhamento a área responsável com motivo, fontes e resposta. No query endpoint, o BC-01 apenas **indica** o escalonamento (TP-40); a execução é do BC-03 (DP-09). | BC-03 | Apenas sugerir "procure seu supervisor" |


**Termos do produto acrescentados na versão final.** Definem a linguagem do BC-01 usada pelo requirements.

| ID | Termo | Definição | Context | Interpretação incorreta provável |
| --- | --- | --- | --- | --- |
| TP-11 | Domínio do assistente | Temas cobertos pelos contexts de negócio BC-05 a BC-11. Dúvida fora deles é S9. | BC-01 | Limitar o domínio às quatro categorias do cenário |
| TP-12 | Categoria de dúvida | Agrupamento do cenário: prazo de entrega, regras de frete, devolução, SLA. Não delimita o domínio. | BC-01 | Rejeitar dúvida sobre seguro ou carga perigosa como fora do domínio |
| TP-13 | Dúvida | Mensagem do atendente submetida ao assistente. | BC-01 | Tratar pedido de ação como dúvida |
| TP-14 | Parte | Pergunta identificada dentro da dúvida, associada a um context. Tem situação, nível de confiança e alertas próprios. | BC-01 | Uma parte por categoria, fundindo duas perguntas do mesmo context |
| TP-15 | Bloco | Unidade dentro da parte: uma afirmação de regra com citação, uma ausência, uma divergência ou um pedido de dado. | BC-01 | Tratar a parte inteira como uma afirmação |
| TP-16 | Orientação | Resultado devolvido ao atendente para uma dúvida. Rótulo de UI: "Orientação". | BC-01 | Tratar como resposta pronta para o cliente |
| TP-17 | Situação da resposta | S0 indisponível, S1 completa, S2 parcial, S3 sem evidência, S4 baixa evidência, S5 regra prevalente com divergência, S6 conflito não resolvido, S7 informação necessária, S8 ação fora da alçada, S9 fora do domínio. Tabela completa em `06` §3.9. | BC-01 | Confundir situação com nível de confiança |
| TP-18 | Nível de confiança | Alta, Média, Baixa ou Não aplicável, por bloco e agregado por parte. Reflete a qualidade da evidência, não a sensibilidade do tema. | BC-01 | Rebaixar a confiança por ser tema sensível |
| TP-19 | Afirmação explícita | O trecho citado enuncia a regra diretamente, sem combinar trechos, calcular ou ler condição implícita. Exemplo positivo: o prazo de 7 dias úteis da POL-001 §3.1. | BC-01 | Tratar dedução de dois trechos como explícita |
| TP-20 | Regra prevalente | Regra da fonte que prevalece por precedência decidida. Hoje, só formal sobre informal; entre formais, DP-01. | BC-02 / BC-01 | Tratar a versão mais recente como prevalente |
| TP-21 | Bloco independente | Bloco que não depende de documento ausente nem de dado não informado. | BC-01 | Omitir a parte inteira porque um bloco faltou |
| TP-22 | Dependência bloqueante | A pergunta da parte só pode ser respondida com o conteúdo de documento ausente (ex.: valor final depende da tabela mensal; tarifa de carga perigosa depende da PROC-043). Exige indicação de escalonamento. | BC-01 | Tratar toda ausência como bloqueante |
| TP-23 | Dado do caso | Informação do atendimento que decide a regra: peso, tier, região de destino, tipo e classe da carga, data de abertura do chamado, data do contrato, CT-e (lista do BC-01). | BC-01 | Confundir com dado pessoal |
| TP-24 | Pedido de ação | Pedido para autorizar, conceder ou aprovar algo. Situação S8. | BC-01 | Responder com linguagem de aprovação |
| TP-25 | Citação | Documento, título, seção, versão, data de atualização, classificação e trecho literal (ou referência de célula de tabela). | BC-01 | Citar só o documento |
| TP-26 | Trecho literal | Texto do documento sem tradução nem paráfrase, comparado após a normalização definida no requirements (C-38). | BC-01 | Parafrasear entre aspas |
| TP-27 | Data da fonte | Data declarada no cabeçalho do documento, exibida com o rótulo do próprio documento: "Última atualização" (POL-001, SLA-2024), "Data de emissão" (PROC-042 v1 e v2), "Última atualização: Diversas" (FAQ). Não é data de ingestão. | BC-02 | Exibir "Atualizado em" para o PROC-042, que só declara emissão |
| TP-28 | Metadado ausente | Versão ou data não informada na fonte (ex.: FAQ "não controlada"). Exibido com sentinela. | BC-02 | Inventar versão ou data |
| TP-29 | Documento ausente | Documento citado por outro, mas fora do corpus: PROC-043, PROC-088, tabela mensal, prazo padrão por rota, frete padrão. | BC-02 | Descrever o conteúdo presumido |
| TP-30 | Coexistência de versões | Duas versões do mesmo documento ativas sem vigência exclusiva (caso do PROC-042). Sempre sinalizada. | BC-02 | Tratar como conflito resolvido pela data |
| TP-31 | Conflito registrado | Conflito documental (TP-05) inscrito no registro do BC-02 (§2.6), por seção. Gera alerta mesmo quando só uma fonte é recuperada. | BC-02 | Exigir que as duas fontes sejam recuperadas para alertar |
| TP-32 | Divergência detectada | Divergência entre trechos recuperados que não consta do registro. Tratada como S6 "Possível divergência" e enviada ao BC-02. | BC-01 → BC-02 | Ignorar ou registrar como conflito oficial |
| TP-33 | Validação humana | Confirmação por especialista antes de comunicar ao cliente (`JORNADA` F2, F3). O assistente só indica a necessidade. | BC-03 | Tratar como opcional |
| TP-34 | Área de destino | Área indicada pela tabela do §2.4. Uma por parte. | BC-03 | "Supervisor" genérico |
| TP-35 | Área padrão de fallback | Área indicada quando o tema não tem destino documentado. Provisório: Supervisão de Atendimento (SAC N2), DP-30. | BC-03 | Deixar a indicação sem área |
| TP-36 | Registro de interação | Registro único de cada orientação: dúvida mascarada, orientação, fontes e versões, situações, níveis, alertas, áreas, identificador. | BC-01 | Manter registros paralelos no BC-03 e no BC-04 |
| TP-37 | Identificador da orientação | Código único, não reutilizável, sem dado pessoal, exibido em toda orientação. | BC-01 | Usar o número do chamado |
| TP-38 | Feedback | Concordância ou discordância do atendente com a orientação, com os motivos da `JORNADA` R1 (errada, desatualizada, incompleta, ambígua, fonte incorreta, outro). Registrado pelo BC-01; tratado pelo BC-04. | BC-01 / BC-04 | Tratar feedback como escalonamento |
| TP-39 | Contestação | Feedback de discordância que entra no ciclo de avaliação e correção do BC-04 (`JORNADA` R3 a R5). | BC-04 | Prometer acompanhamento sem fluxo definido |
| TP-40 | Indicação de escalonamento | Sinal da orientação com motivo e área de destino. Não executa o escalonamento (TP-10), que é do BC-03. | BC-01 → BC-03 | Escrever "escalado" ou "enviado ao supervisor" |
| TP-41 | Status de vigência | Situação de vigência exibida em toda citação, exigida pelo G3 da `JORNADA` ("data de vigência"). Nenhum documento do corpus declara data de vigência; os valores possíveis são: "vigente por premissa P-01" (POL-001, SLA-2024), "sem indicação formal de vigência" (PROC-042 v1 e v2, conforme o campo Status de cada um) e "não controlada" (FAQ). | BC-02 | Exibir a data de emissão como se fosse data de vigência |

### 3.4 Termo → rótulo de UI

| Termo | Rótulo no canal | Rótulos que não devem ser usados |
| --- | --- | --- |
| Orientação (TP-16) | "Orientação" | "A resposta está" |
| S1 | "Resposta completa" | — |
| S2 | "Resposta parcial" | — |
| S3 | "Sem evidência na documentação" | — |
| S4 | "Baixa evidência · fonte informal" | "Baixa evidência" como nível de confiança |
| S5 | "Regra prevalente com divergência" | — |
| S6 | "Conflito não resolvido" ou, para TP-32, "Possível divergência" | — |
| S7 | "Informação necessária" | — |
| S8 | "Ação fora da alçada do assistente" | — |
| S9 | "Fora do domínio" | — |
| S0 | "Assistente indisponível" | — |
| Nível (TP-18) | "Confiança Alta / Média / Baixa / Não aplicável" | "Baixa evidência" |
| Validação humana (TP-33) | "Requer validação humana" | — |
| Indicação de escalonamento (TP-40) | "Área indicada: <área>" | "Escalar ao supervisor" (contraria TP-10) |
| Fonte informal (TP-02) | "Fonte informal · não validada por Compliance ou Operações" (texto do cabeçalho do FAQ) | — |
| Feedback (TP-38) | Motivos da `JORNADA` R1 | Lista parcial de motivos |
...
As pendências 1 a 13 continuam abertas. O comportamento provisório do assistente enquanto elas não forem resolvidas está no `06-requirements-final.md` (§4.3 e §4.4). A pendência 9 foi atendida no requirements (§4.2).

### 4.1 Pendências abertas pela revisão do Tech Lead

| ID | Pendência | Afeta | Comportamento provisório |
| --- | --- | --- | --- |
| DT-02 / DP-30 | Confirmar a tabela tema → área e a área padrão | §2.4, TP-34, TP-35 | Tabela do §2.4 |
| DT-10 | Área de carga perigosa: divisão por documento (Gestão de Riscos na devolução, Compliance no frete, tarifa e expresso) ou proposta da Tarefa 1 ("Operações e Compliance", que exige escolher uma área) | §2.4, BC-07 | Linhas 1 e 2 do §2.4 |
| DT-11 / DP-12 | Definir qual chamado fornece a data de referência do PROC-042 | T-41, TP-04, T-12 | Data de abertura informada pelo atendente para o chamado relacionado ao frete |
| DP-08 | Níveis N1, N2 e N3 por tema sensível | §2.5 | Alerta em todo o baseline |
| DP-01 | Hierarquia entre fontes formais | TP-20 | Conflito entre formais é S6 |
